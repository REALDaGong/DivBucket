package android.tj007.divbucketmvp.view.activity.view;
...
import android.tj007.divbucketmvp.utils.BitmapUtil;
...
public class ClipViewLayout extends RelativeLayout {
    ...
    public void initSrcPic(Uri uri) {
        ...
        Log.d("bit", "**********clip_view uri*******  " + uri);
        String path = getRealFilePathFromUri(getContext(), uri);
        Log.d("bit", "**********clip_view path*******  " + path);
        if (TextUtils.isEmpty(path)) {
            return;
        }

        //这里decode出720*1280 左右的照片,防止OOM
        Bitmap bitmap = BitmapUtil.getBitmapFromUri(getContext(),uri);
        //Bitmap bitmap = decodeSampledBitmap(path, 720, 1280);
        Log.d("bit", "**********decode*******  ");
        if (bitmap == null) {
            Log.d("bit", "**********decode null*******  ");
            return;
        }
